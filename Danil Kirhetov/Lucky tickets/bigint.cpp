...
//Для того, чтоб не было переполнения переменной при вычислении больших чисел (>10е19)
//был использован класс для работы с большими числами
...
big_integer Combine(int n, int l) {                     //функция вычисления биномиальных коэфициентов (количества сочетаний)
    std::vector<big_integer> C;
    for (int i = 0; i <= l + 1; i++) C.push_back(0);

    C[0] = 1;
    for (int i = 0; i <= n; i++) {
        for (int j = std::min(i, l); j > 0; j--) {
            C[j] = C[j] + C[j - 1];
        }
    }
    return C[l];
...
int main() {
    ...
    big_integer Integral = 0;
    big_integer mod ("18446744073709551616");           //ограничение по модулю

    for (int i = 0; i < K; i++) {
        if (i % 2 == 0) {
            Integral = Integral + Combine(n - (i * p), K) * Combine(2 * k, i);
        }
        else {
            Integral = Integral - Combine(n - (i * p), K) * Combine(2 * k, i);
        }
    ...
    //Данная програма производит правильные вычисления. Проблема лишь в том, что для 
    //больших чисел время подсчета превышает 2 секунды, когда требуемое время вычисления
    //не более 500 мс

    std::cout << Integral % mod;
